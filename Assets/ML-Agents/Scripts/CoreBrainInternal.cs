...
// there are multiple internal brains
        ...
        if (graphModel != null)
        {
            ...
            if (graphScope.Length > 1 && graphScope[graphScope.Length - 1] != '/') graphScope = graphScope + '/';

            if (graph[graphScope + BatchSizePlaceholderName] != null) hasBatchSize = true;
            ...
            if (graph[graphScope + VectorObservationPlacholderName] != null) hasState = true;
            if (graph[graphScope + PreviousActionPlaceholderName] != null) hasPrevAction = true;
        }
    ...
    public void DecideAction(Dictionary<Agent, AgentInfo> agentInfo)
    {
        ...
        if (hasState)
        {
            ...
            foreach (var agent in agentList)
            {
                ...
                for (var j = 0; j < stateLength * brain.brainParameters.numStackedVectorObservations; j++)
        ...
        for (var observationIndex = 0;
            observationIndex < brain.brainParameters.cameraResolutions.Count();
            observationIndex++)
        {
            texturesHolder.Clear();
            foreach (var agent in agentList) texturesHolder.Add(agentInfo[agent].visualObservations[observationIndex]);
        ...
        if (hasRecurrent)
        {
            ...
            foreach (var agent in agentList)
            {
                ...
                for (var j = 0; j < m.Count(); j++) inputOldMemories[i, j] = m[j];
                i++;
            }
        }
        ...
        if (hasBatchSize) runner.AddInput(graph[graphScope + BatchSizePlaceholderName][0], new[] {currentBatchSize});

        foreach (var placeholder in graphPlaceholders)
            try
            {
                if (placeholder.valueType == TensorFlowAgentPlaceholder.tensorType.FloatingPoint)
                    runner.AddInput(graph[graphScope + placeholder.name][0],
                        new[] {Random.Range(placeholder.minValue, placeholder.maxValue)});
                else if (placeholder.valueType == TensorFlowAgentPlaceholder.tensorType.Integer)
                    runner.AddInput(graph[graphScope + placeholder.name][0],
                        new[] {Random.Range((int) placeholder.minValue, (int) placeholder.maxValue + 1)});
            }
            catch
            {
        ...
        if (hasState)
        {
            if (brain.brainParameters.vectorObservationSpaceType == SpaceType.discrete)
            {
                ...
                for (var i = 0; i < currentBatchSize; i++) discreteInputState[i, 0] = (int) inputState[i, 0];
        ...
        if (hasPrevAction) runner.AddInput(graph[graphScope + PreviousActionPlaceholderName][0], inputPrevAction);

        // Create the observation tensors
        for (var obs_number = 0; obs_number < brain.brainParameters.cameraResolutions.Length; obs_number++)
        ...
        catch (TFException e)
        {
            ...
            try
            {
                errorMessage = string.Format(@"The tensorflow graph needs an input for {0} of type {1}",
                    e.Message.Split(new[] {"Node: "}, 0)[1].Split('=')[0],
                    e.Message.Split(new[] {"dtype="}, 0)[1].Split(',')[0]);
            }
            finally
            {
        ...
        if (hasRecurrent)
        {
            ...
            foreach (var agent in agentList)
            {
                float[] m = new float[memorySize];
                for (var j = 0; j < memorySize; j++) m[j] = recurrent_tensor[i, j];
        ...
        if (brain.brainParameters.vectorActionSpaceType == SpaceType.continuous)
        {
            ...
            foreach (var agent in agentList)
            {
                float[] a = new float[brain.brainParameters.vectorActionSize];
                for (var j = 0; j < brain.brainParameters.vectorActionSize; j++) a[j] = output[i, j];
        ...
        else if (brain.brainParameters.vectorActionSpaceType == SpaceType.discrete)
        {
            ...
            foreach (var agent in agentList)
            {
                float[] a = new float[1] {output[i, 0]};
                agent.UpdateVectorAction(a);
                i++;
            }
        }
    ...
    public void OnInspector()
    {
        ...
        graphScope = EditorGUILayout.TextField(new GUIContent("Graph Scope",
        ...
        if (BatchSizePlaceholderName == "") BatchSizePlaceholderName = "batch_size";
        BatchSizePlaceholderName = EditorGUILayout.TextField(new GUIContent("Batch Size Node Name",
            "If the batch size is one of " +
            "the inputs of your graph, you must specify the name if the placeholder here."), BatchSizePlaceholderName);
        if (VectorObservationPlacholderName == "") VectorObservationPlacholderName = "state";
        VectorObservationPlacholderName = EditorGUILayout.TextField(new GUIContent("Vector Observation Node Name",
        ...
        if (RecurrentInPlaceholderName == "") RecurrentInPlaceholderName = "recurrent_in";
        RecurrentInPlaceholderName = EditorGUILayout.TextField(new GUIContent("Recurrent Input Node Name",
            "If your graph uses a " +
        ...
        if (RecurrentOutPlaceholderName == "") RecurrentOutPlaceholderName = "recurrent_out";
        RecurrentOutPlaceholderName = EditorGUILayout.TextField(new GUIContent("Recurrent Output Node Name",
            " If your graph uses a " +
        ...
        if (brain.brainParameters.cameraResolutions != null)
            if (brain.brainParameters.cameraResolutions.Count() > 0)
            {
                ...
                if (VisualObservationPlaceholderName.Count() != brain.brainParameters.cameraResolutions.Count())
                    VisualObservationPlaceholderName = new string[brain.brainParameters.cameraResolutions.Count()];
                for (var obs_number = 0; obs_number < brain.brainParameters.cameraResolutions.Count(); obs_number++)
        ...
        if (ActionPlaceholderName == "") ActionPlaceholderName = "action";
        ActionPlaceholderName = EditorGUILayout.TextField(new GUIContent("Action Node Name",
    ...
    private bool hasRecurrent;
    private bool hasState;
    private bool hasBatchSize;
    private bool hasPrevAction;
    private float[,] inputState;
    private int[] inputPrevAction;
    private List<float[,,,]> observationMatrixList;
    private float[,] inputOldMemories;
    private List<Texture2D> texturesHolder;
