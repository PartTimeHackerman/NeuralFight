...
{
    private readonly Dictionary<Agent, AgentInfo> agentInfos =
    ...
    [SerializeField] private ScriptableObject[] CoreBrains;
    ...
    public void UpdateCoreBrains()
    {
        ...
        if (CoreBrains == null)
        {
            var numCoreBrains = Enum.GetValues(typeof(BrainType)).Length;
            CoreBrains = new ScriptableObject[numCoreBrains];
            foreach (BrainType bt in Enum.GetValues(typeof(BrainType)))
                CoreBrains[(int) bt] =
        ...
        if (CoreBrains.Length < Enum.GetValues(typeof(BrainType)).Length)
        {
            var numCoreBrains = Enum.GetValues(typeof(BrainType)).Length;
