...
public class ExternalCommunicator : Communicator
{
    ...
    private const int defaultNumAgents = 32;
    private const int defaultNumObservations = 32;

    private const string _version_ = "API-3";
    private readonly Academy academy;

    private readonly List<Brain> brains;

    private ExternalCommand command = ExternalCommand.QUIT;


    private int comPort;

    private readonly Dictionary<string, List<Agent>> current_agents;
    private readonly Dictionary<string, bool> hasSentState;
    private byte[] lengthHolder;
    private string logPath;

    private StreamWriter logWriter;
    private byte[] messageHolder;
    private int randomSeed;

    private AgentMessage rMessage;
    private readonly StringBuilder rMessageString = new StringBuilder(messageLength);
    private Socket sender;

    private StepMessage sMessage;
    private string sMessageString;
    private readonly Dictionary<string, bool> triedSendState;
    ...
    public void InitializeCommunicator()
    {
        ...
        foreach (var b in brains)
        {
            ...
            if (b.brainType == BrainType.External) accParamerters.externalBrainNames.Add(b.gameObject.name);
        }
        ...
        foreach (var k in accParamerters.brainNames)
        {
    ...
    {
        var location = sender.Receive(messageHolder);
        var message = Encoding.ASCII.GetString(messageHolder, 0, location);
        switch (message)
        {
            case "STEP":
    ...
    public void UpdateActions()
    {
        ...
        foreach (var brain in brains)
            if (brain.brainType == BrainType.External)
            {
                ...
                if (current_agents[brainName].Count() == 0) continue;
                var memorySize = rMessage.memory[brainName].Count() / current_agents[brainName].Count();

                for (var i = 0; i < current_agents[brainName].Count(); i++)
                {
                    if (brain.brainParameters.vectorActionSpaceType == SpaceType.continuous)
                        current_agents[brainName][i].UpdateVectorAction(rMessage.vector_action[brainName].GetRange(
                                i * brain.brainParameters.vectorActionSize, brain.brainParameters.vectorActionSize)
                            .ToArray());
                    else
                        current_agents[brainName][i]
                            .UpdateVectorAction(rMessage.vector_action[brainName].GetRange(i, 1).ToArray());
    ...
    private void ReadArgs()
    {
        ...
        {
            if (args[i] == "--port") inputPort = args[i + 1];
            if (args[i] == "--seed") inputSeed = args[i + 1];
        }
    ...
    {
        var envMessage = JsonConvert.SerializeObject(envParams, Formatting.Indented);
    ...
    private void ReceiveAll()
    {
        ...
        var totalLength = BitConverter.ToInt32(lengthHolder, 0);
        var location = 0;
        rMessageString.Clear();
        while (location != totalLength)
        {
            var fragment = sender.Receive(messageHolder);
            location += fragment;
    ...
    private byte[] AppendLength(byte[] input)
    {
        ...
        BitConverter.GetBytes(input.Length).CopyTo(newArray, 0);
        return newArray;
    }
    ...
    public void GiveBrainInfo(Brain brain, Dictionary<Agent, AgentInfo> agentInfo)
    {
        ...
        foreach (var agent in agentInfo.Keys) current_agents[brainName].Add(agent);
        if (current_agents[brainName].Count() > 0)
        {
            ...
            var memorySize = 0;
            foreach (var agent in current_agents[brainName])
            ...
            foreach (var agent in current_agents[brainName])
            {
                ...
                for (var j = 0; j < memorySize - agentInfo[agent].memories.Count; j++) sMessage.memories.Add(0f);
            ...
            Receive();
            var i = 0;
            foreach (var res in brain.brainParameters.cameraResolutions)
            {
                foreach (var agent in current_agents[brainName])
                {
        ...
        if (triedSendState.Values.All(x => x))
        {
            if (hasSentState.Values.Any(x => x) || academy.IsDone())
            {
                ...
                sender.Send(
                    AppendLength(Encoding.ASCII.GetBytes("END_OF_MESSAGE:" + (academy.IsDone() ? "True" : "False"))));


                UpdateCommand();
                if (GetCommand() == ExternalCommand.STEP) UpdateActions();
            }

            foreach (var k in current_agents.Keys)
            {
                hasSentState[k] = false;
                triedSendState[k] = false;
            }
    ...
    private struct ResetParametersMessage
    {
