...
{
    [SerializeField] private bool _Foldout = true;
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (myBrain.transform.parent == null)
            ...
            EditorGUILayout.HelpBox("The Parent of a Brain must have an Academy Component attached to it!",
                MessageType.Error);
        ...
        if (_Foldout)
        {
            ...
            var bpVectorObsType = serializedBrain.FindProperty("brainParameters.vectorObservationSpaceType");
            ...
            var bpVectorObsSize = serializedBrain.FindProperty("brainParameters.vectorObservationSize");
            ...
            var bpNumStackedVectorObs = serializedBrain.FindProperty("brainParameters.numStackedVectorObservations");
            EditorGUILayout.PropertyField(bpNumStackedVectorObs, new GUIContent("Stacked Vectors",
                "Number of states that" +
                " will be stacked before beeing fed to the neural network."));

            EditorGUI.indentLevel--;
            var bpCamResol = serializedBrain.FindProperty("brainParameters.cameraResolutions");
            EditorGUILayout.PropertyField(bpCamResol, new GUIContent("Visual Observation", "Describes height, " +
                                                                                           "width, and whether to greyscale visual observations for the Brain."),
                true);

            EditorGUILayout.LabelField("Vector Action");
            EditorGUI.indentLevel++;

            var bpVectorActionType = serializedBrain.FindProperty("brainParameters.vectorActionSpaceType");
            EditorGUILayout.PropertyField(bpVectorActionType, new GUIContent("Space Type",
                "Corresponds to whether state" +
                " vector contains a single integer (Discrete) " +
                "or a series of real-valued floats (Continuous)."));

            var bpVectorActionSize = serializedBrain.FindProperty("brainParameters.vectorActionSize");
            ...
            var bpVectorActionDescription = serializedBrain.FindProperty("brainParameters.vectorActionDescriptions");
            EditorGUILayout.PropertyField(bpVectorActionDescription, new GUIContent("Action Descriptions",
                "A list of strings used to name" +
        ...
        if (bt.enumValueIndex < 0) bt.enumValueIndex = (int) BrainType.Player;
